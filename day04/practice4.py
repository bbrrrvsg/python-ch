...
# 문제 1: range와 역순 출력 range() 함수와 reversed() 함수를 사용하여
# 10부터 1까지의 숫자를 역순으로 한 줄에 하나씩 출력하는 프로그램을 작성하시오.

for i in reversed(range(1,11)):
    print(i)

for i in range(10,0,-1): # 끝 값 미포함 0미포함하고 1까지 
    print(i)
...
fruits = ['사과', '바나나', '딸기', '포도']
for i,k in enumerate(fruits):
    print(f"{i}번 과일: {k}", end=", ")
print()

# 문제 4: 리스트 내포(List Comprehension) 1부터 10개까지의 숫자 중 홀수만 골라
# 그 제곱값을 담는 리스트를 리스트 내포 방식을 사용하여 한 줄로 생성하고 출력하시오.
# (결과 예시: [1, 9, 25, 49, 81])
...
result1 = [i for i in range(1,21) if i%2 != 0 and i%3 != 0]
print(result1)
...
while True:
    aput = int(input("숫자 입력 :"))
    if(aput==0):
        print("프로그램 종료")
        break


# 문제 7: 중첩 반복문 (구구단) 중첩 for문을 사용하여 구구단 2단부터 9단까지 출력하시오. 
...
# 문제 8: 시각화 보고서 (매출 그래프) 아래 리스트는 4주간의 매출 데이터입니다. for문과 enumerate()를 사용하여 백만원 단위마다 
# ★ 문자로 시각화하여 보고서를 출력하시오.
...
for i in range(len(car_numbers)):
    pay = basicp+((usage_minutes[i]-30)//10)*500
    if pay > 20000:
        pay = 20000
    print(f"{car_numbers[i]}: {usage_minutes[i]}분 주차, 최종요금: {pay}원")
